...
class CenteredCamera(Camera):
    ...
    def get_window_viewport(self):
        ...
        default_offset_x = self._window.width / 2
        default_offset_y = self._window.height / 2

        x = default_offset_x- (offset_x * zoom)
        y =  default_offset_y - (offset_y * zoom)
        width = self._window.width / zoom
        height = self._window.height / zoom
        return (offset_x - default_offset_x/zoom, offset_y - default_offset_y/zoom, width, height, zoom)
    ...
    def contains(self, x, y):
        ...
        s_x = x * zoom + default_offset_x - offset_x * zoom
        s_y = y * zoom + default_offset_y - offset_y * zoom
        contains = s_x > 0 and s_x < self._window.width and s_y > 0 and s_y < self._window.height
        return contains

    def contains_circle(self, x, y, circle):
        zoom = self.zoom
        offset_x = self.offset_x
        offset_y = self.offset_y
        default_offset_x = self._window.width / 2
        ...
        s_x = circle.x * zoom + default_offset_x - offset_x * zoom
        s_y = circle.y * zoom + default_offset_y - offset_y * zoom
        if (x - s_x) ** 2 + (y - s_y) ** 2 <= (circle.radius * zoom) ** 2:
            return True
        else:
            return False

    def contains_rectangle(self, x, y, rectangle):
        zoom = self.zoom
        offset_x = self.offset_x
        offset_y = self.offset_y
        default_offset_x = self._window.width / 2
        ...
        x1 = rectangle.x * zoom + default_offset_x - offset_x * zoom
        y1 = rectangle.y * zoom + default_offset_y - offset_y * zoom
        x2 = x1 + width
        y2 = y1 + height
